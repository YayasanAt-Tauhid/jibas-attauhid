...
- **Ditemukan & ditindak saat sinkronisasi ini: `scripts/.env` ter-commit di git sejak commit `41813d6`, berisi service_role key (bypass RLS penuh) untuk project V1 lama (`leyfwwmroijwnkrcblxe`), berlaku sampai 2036.** Atas arahan user: file di-`git rm --cached` (berhenti di-track, tetap ada di disk untuk lokal; `.gitignore` sudah punya aturan `.env` yang sekarang efektif). **BELUM dilakukan:** key tsb belum di-revoke di Supabase Dashboard, dan key itu masih ada di git history lama (commit sebelum penghapusan) — kalau project V1 masih hidup, key itu tetap valid sampai direvoke manual
