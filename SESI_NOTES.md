...
- **Konfigurasi Supabase di repo belum disinkronkan dengan project aktif V4** — `supabase/config.toml` (`project_id`) dan `.env.example` masih menunjuk ke `rumdeqkrtfjxckqgokoy` (V2 lama). Perlu diupdate ke `oquzygbekjpbmafuqiot`, dan `scripts/.env` (masih menunjuk `leyfwwmroijwnkrcblxe`, V1) juga perlu ditinjau
