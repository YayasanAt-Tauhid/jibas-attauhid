...
- **Terapkan migrasi `20260716080000_perangkat_push_ortu.sql` ke produksi** (`supabase db push`) — MCP Supabase sesi ini hanya melihat project asing `nxegugfgzayjnyqagcge` (bukan `leyfwwmroijwnkrcblxe`), jadi migrasi belum diterapkan; sebelum itu push notification gagal best-effort (RPC `daftarkan_push_token` tidak ada) tanpa mengganggu alur lain
