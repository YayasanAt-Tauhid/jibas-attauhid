-- ─────────────────────────────────────────────────────────────────────────────
-- generate_tagihan_batch: hanya posting jurnal untuk tagihan yang SUDAH jatuh
-- tempo; tagihan periode mendatang cukup dijadwalkan.
--
-- Sebelumnya setiap tagihan yang di-generate langsung memposting
--     (D) Piutang Siswa   (K) Pendapatan
-- tanpa memandang kapan periode tagihannya. Untuk sekolah yang meng-input SPP
-- 6 tahun ke depan sekaligus, itu berarti mengakui pendapatan 6 tahun di muka.
--
-- Sekarang:
--   * jatuh_tempo > hari ini  -> tagihan disimpan dengan status 'terjadwal',
--                                jurnal_piutang_id NULL, TIDAK ada jurnal.
--   * jatuh_tempo <= hari ini -> perilaku lama: jurnal piutang diposting dan
--                                status 'belum_bayar'.
--
-- Tagihan 'terjadwal' tetap terlihat & bisa dibayar orang tua (lihat migrasi
-- view v_tagihan_belum_bayar); pembayaran sebelum jatuh tempo dibukukan sebagai
-- Pendapatan Diterima di Muka, bukan pelunasan piutang.
--
-- Perpindahan 'terjadwal' -> 'belum_bayar' + jurnal piutangnya dikerjakan oleh
-- RPC posting_piutang_jatuh_tempo (migrasi 20260728100002).
--
-- Tanggal jurnal tetap current_date (bukan jatuh_tempo) agar migrasi ini tidak
-- diam-diam menyisipkan jurnal bertanggal mundur ke bulan yang laporannya sudah
-- terbit / bukunya sudah ditutup. Tanggal jatuh tempo yang sebenarnya tersimpan
-- di kolom tagihan.jatuh_tempo sehingga laporan umur piutang tetap presisi.
-- ─────────────────────────────────────────────────────────────────────────────

-- Return type bertambah kolom `scheduled`, jadi fungsi lama harus di-DROP dulu
-- (Postgres tidak mengizinkan CREATE OR REPLACE yang mengubah tipe kembalian).
-- DROP dengan signature argumen yang PERSIS supaya tidak menyisakan overload
-- lama yang diam-diam ikut terpanggil -- pola bug yang sudah pernah terjadi di
-- get_tarif_siswa dan buku_besar_mutasi.
DROP FUNCTION IF EXISTS public.generate_tagihan_batch(uuid, uuid, integer, uuid, jsonb, uuid);

CREATE FUNCTION public.generate_tagihan_batch(
  p_jenis_id uuid,
